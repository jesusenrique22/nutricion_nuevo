...
import { submitAnthropometryForm } from "@/server/actions/anthropometry.actions";
...
export function AnthropometryFormClient({
  appointmentId,
  patientEmail,
  appointmentLabel,
}: {
  ...
  function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const res = await submitAnthropometryForm(appointmentId, payload);
      if (!res.ok) {
    ...
    <form ref={formRef} onSubmit={handleSubmit}>
      ...
      {step === 1 && (
        ...
        <FormSection
          ...
          description="Gracias por elegirme. Toda la información será tratada con máxima confidencialidad."
        >
          <Field label="Email">
            <input
              type="email"
          ...
          <p className="text-sm text-foreground/70">
            Puedes ver en qué consiste la evaluación consultando los{" "}
            <span className="font-semibold text-primary">puntos anatómicos</span>{" "}
            que se medirán durante la sesión.
          </p>
          ...
          <div className="rounded-xl bg-muted/60 p-4 text-sm text-foreground/80">
            ...
            <ol className="mt-2 list-decimal space-y-2 pl-5">
              ...
              <li>
                Mis datos personales serán protegidos conforme a la normativa
          ...
          <label className="flex items-start gap-3 rounded-xl border border-foreground/15 p-4">
            ...
            <span className="text-sm">
              He leído y comprendido la información sobre el procedimiento de
              evaluación antropométrica y doy mi consentimiento para participar.{" "}
      ...
      {step === 4 && (
        <div data-step="4">
        <FormSection title="Preferencias de evaluación">
      ...
      {step === 5 && (
        ...
        <FormSection title="Recordatorio y preparación">
          <div className="rounded-xl bg-accent/10 p-4 text-sm text-foreground/80">
            ...
            <ol className="mt-2 list-decimal space-y-2 pl-5">
              <li>
                Evitar alimentos y bebidas (excepto agua) al menos 2 horas antes
                de la evaluación.
              </li>
              <li>
                No realizar ejercicio intenso 24 horas antes de la medición.
              </li>
              <li>
                Usar ropa ligera y ajustada el día de la evaluación (ej. short /
                top deportivo).
              </li>
              <li>Informar cualquier cambio reciente en tu estado de salud.</li>
              <li>
                Informar consumo de creatina, especialmente si acabas de iniciar
                suplementación.
              </li>
            </ol>
            <p className="mt-3 text-xs text-foreground/60">
              Los puntos 1 y 2 no son excluyentes para realizar la evaluación,
              pero siguen las pautas ISAK para resultados más precisos.
            </p>
          </div>
